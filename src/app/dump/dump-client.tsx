...
export function DumpClient({ initialRecent }: { initialRecent: DumpResult[] }) {
  const [input, setInput] = useState("");
  const [processing, setProcessing] = useState(false);
  const [results, setResults] = useState<DumpResult[]>([]);
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [recentTasks, setRecentTasks] = useState<DumpResult[]>(initialRecent);
  ...
  const allResults = [
    ...results,
    ...recentTasks.filter((r) => !results.some((s) => s.id === r.id)),
  ];
    ...
    <>
      ...
      {allResults.length > 0 && (
        <div className="space-y-2">
          <p className="mono-tag-muted px-1">Filed Tasks</p>
          {allResults.map((result) => {
            ...
            return (
              ...
              >
                ...
                >
                  <CheckCircle2 className="size-4 text-[var(--chart-2)] shrink-0" />
                  ...
                  <Badge
                    variant="secondary"
                    className={cn(
                      "text-[10px] px-1.5 py-0 rounded border shrink-0",
                ...
                {isExpanded && (
                  <div className="px-3 pb-3 pt-0 border-t border-border/30 space-y-3">
                    ...
                    {result.research && (
                      <>
                        <p className="mono-tag-muted">AI Research &amp; Tips</p>
