...
- [x] **Pre-SaaS blocker — PARTIALLY CLOSED by tenancy foundation (2026-07-01):** all authenticated CRM + proposal routes now run on the user-scoped client with membership RLS (see Tenancy Foundation section). REMAINING: the non-CRM service-role routes (`/api/leads/*`, `/api/seo/clients`, `/api/audits/list`, `/api/issues`, `/api/ai/execute`, `/api/upload`) still query by user-supplied id. Under the new membership login gate a client login CAN reach them — hide those pages from client roles or scope them before onboarding a real client login. Next hardening slice.
...
#### Tenancy Foundation (2026-07-01) — `COMPLETE` (branch feat/tenancy, PR pending review)
Spec: `docs/superpowers/specs/2026-07-01-tenancy-foundation-design.md` · Plan: `docs/superpowers/plans/2026-07-01-tenancy-foundation.md`
- [x] **Migration 040 (APPLIED live)** — `workspaces.kind` ('internal'/'client'/'demo') + `branding` jsonb stub; `workspace_members` (workspace_id, user_id, role); `agency_admins` (justin@, dillon@; RLS deny-all); SECURITY DEFINER helpers `is_agency_admin` / `is_workspace_member` (authenticated-only EXECUTE — anon revoked to kill an RPC membership oracle); auto-owner trigger on workspace INSERT; membership backfill. All policies on 9 CRM tables + workspaces dropped dynamically (live names differed from files) and recreated membership-based with `(SELECT auth.uid())`.
- [x] **Demo tenant seeded** — 8 companies / 15 contacts / 10 deals (all 7 stages) / 6 activities / 4 tasks / 2 proposals + 6 line items, all fictional (.example). `admin@niewdel.com` = member of demo ONLY. Reseedable via `supabase/seed-demo-workspace.sql`.
- [x] **RLS probes (ran twice: post-migration + post-refactor)** — admin@ JWT: demo-only, niewdel leak = 0; justin@ JWT: all 4 workspaces; anon REST: `[]` on all 6 tables, INSERT → 42501.
- [x] **Route refactor** — all authenticated `/api/pipeline/*` + `/api/proposals/[id]/countersign` moved from service-role `getPipelineClient()` to cookie-authenticated `getUserScopedClient()` + `resolveActiveWorkspace()` (401 before any DB access). Service role remains ONLY in token-gated `/api/proposals/[id]/{view,sign}`, `/api/portal/*`, webhooks/cron, and promote's leads-table reads.
- [x] **Tenancy lib** — `src/lib/tenancy/` (client wrapper, active-workspace resolution from `active_workspace` cookie validated against RLS-visible list, client-safe constants, fail-closed `isLoginAllowed` gate helper). 11 unit tests.
- [x] **Workspace switcher** — `POST /api/tenancy/switch` (200/400/401/403, non-httpOnly cookie by design — server re-validates every request) + sidebar dropdown (hidden <2 memberships, kind badge for non-internal, mount-gated label).
- [x] **Login gate** — middleware admits CORE_EMAILS/env list OR ≥1 workspace membership (RLS-scoped query, explicitly fail-closed); zero-membership users → `/login?error=no-access` with static message. admin@ now enters purely via Demo membership.
- [x] **Verification** — tsc clean, 323/323 vitest, build green; per-task subagent reviews + 2 fix waves (switcher cookie constant + hydration; explicit fail-closed gate + tests); dev-server smoke: unauth API/page → 307 login, no-access message renders.
